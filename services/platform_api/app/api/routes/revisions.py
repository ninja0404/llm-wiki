...
from ...core.deps import AuthContext, get_workspace_conn, require_auth
...
async def list_revisions(document_id: str, auth: Annotated[AuthContext, Depends(require_auth)]) -> dict:
    for workspace_id in auth.workspace_roles or {}:
        async with acquire(workspace_id) as connection:
            exists = await connection.fetchval("SELECT 1 FROM documents WHERE id = $1::uuid", document_id)
            if not exists:
                continue
...
async def list_workspace_revisions(
    ...
    if workspace_id not in (auth.workspace_roles or {}):
