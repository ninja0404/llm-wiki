...
export default async function VaultPage() {
  ...
  const documents = firstWorkspace
    ? await apiFetch<{ data: { id: string; path: string; kind: string; title: string; status: string }[] }>(
        `/v1/workspaces/${firstWorkspace.id}/documents`
      ).catch(() => ({ data: [] }))
    : { data: [] };
  ...
  return (
    <section className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Vault Explorer</h1>
        <p className="text-sm text-slate-500 mt-1">
          Unified document model for sources, system docs, wiki pages, and assets.
        </p>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50/80">
                ...
                <TableHead className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Kind</TableHead>
              ...
              {documents.data.length === 0 ? (
                ...
                documents.data.map((doc) => (
                  <TableRow key={doc.id} className="hover:bg-muted/50 cursor-pointer">
                    ...
                    <TableCell>
                      <Badge variant={kindVariant[doc.kind] ?? "outline"}>{doc.kind}</Badge>
                    </TableCell>
