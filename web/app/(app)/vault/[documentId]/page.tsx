  ...
  if (!document.data) {
    return (
      <section className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
        <Link href="/vault" className="inline-flex items-center gap-1.5 -ml-2 px-2.5 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-all">
          <ArrowLeft size={16} />
          Back to Vault
        </Link>
        <h1 className="text-2xl font-bold text-slate-900">Document Not Found</h1>
        <p className="text-sm text-slate-500">The requested vault document does not exist or is not accessible.</p>
      </section>
    );
  }
