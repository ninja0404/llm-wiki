...
- [ ] 数据库索引补全 — HNSW 向量索引 + GIN FTS 索引 + GIN trigram 索引
- [ ] 搜索性能优化 — FTS 降级兜底 (embedding IS NULL) + 性能 SLA 验证
- [ ] Lint Worker 实现 — 矛盾检测/过时信息/孤立页面/断裂链接检查
- [ ] MinIO 文件上传集成 — 文件类型 Source 的完整上传/下载链路
- [ ] 生产环境配置 — CORS/Auth baseURL 环境变量化 + 加密密钥校验
- [ ] Dockerfile + docker-compose.prod.yml — API + Worker + Web 容器化
- [ ] 前端 UI 升级 — shadcn/ui 组件替换原始 Tailwind + 响应式优化
- [ ] Wiki 页面版本历史 — 版本 diff 查看 UI
- [ ] Source 级 Job Group — 同一文档批次串行保证 (当前缺失)
- [ ] 健康检查增强 — /health 验证 DB + Redis 连接状态
- [ ] Workspace systemPrompt — Chat RAG 中使用 workspace 级自定义 prompt

### Out of Scope

- 多 LLM Provider Fallback — Phase 1 范围
- SSO/SAML — Phase 2 范围
- 开放 API + API Key 管理 + Scalar 文档 — Phase 2 范围
- 私有化部署 Helm Chart — Phase 2 范围
- Embedding 模型迁移工具 — Phase 1 范围
- 计费系统 — Phase 2 范围
- 可观测性升级 (OpenTelemetry) — Phase 1 范围
- 审计表分区 — Phase 1 范围
- 测试基础设施 — 暂不在本轮范围内 (虽然重要，但聚焦补齐功能)
- Query 缓存 (Redis) — Phase 1 范围
